...
# 日付タブのラベル形式（"3/17" "12/31" など）
_DATE_LABEL_RE = re.compile(r"^\d{1,2}/\d{1,2}$")
...
def _parse_date_label(label: str, today: _date_cls | None = None) -> str | None:
    """
    "3/17" → "20260317" に変換する。
    年をまたぐ場合は today を基準に直近の日付を採用する。

    Returns:
        "YYYYMMDD" 文字列、または解析失敗時 None
    """
    if today is None:
    ...
    if not m:
        return None
    month, day = int(label.split("/")[0]), int(label.split("/")[1])
    for year_offset in (0, -1):
        try:
            candidate = _date_cls(today.year + year_offset, month, day)
            if candidate <= today:
...
def collect_date_tabs(page: Page) -> list[dict]:
    """
    現在ページに表示されている日付タブを収集する（>> は押さない）。

    Returns:
        [{"label": "3/17", "date_str": "20260317", "locator": <Locator>}, ...]
        新しい日付順にソート済み
    """
    today = _date_cls.today()
    tabs: list[dict] = []
            ...
            for elem in page.locator(sel).all():
                try:
                    if not elem.is_visible(timeout=200):
                        continue
                    text = elem.inner_text(timeout=300).strip()
                    if not _DATE_LABEL_RE.match(text):
                        continue
                    date_str = _parse_date_label(text, today)
                    if date_str and date_str not in seen:
                        seen.add(date_str)
                        tabs.append({"label": text, "date_str": date_str, "locator": elem})
                except Exception:
                    continue
        except Exception:
            continue

    tabs.sort(key=lambda t: t["date_str"], reverse=True)
    return tabs
...
def click_date_tab(
    ...
    """
    target_date (YYYYMMDD) に対応する日付タブをクリックする。
    表示中の4日になければ >> を最大2回押してさらに探す。

    Returns:
        True : クリック成功（その日のデータが表示された状態）
        False: 見つからなかった（スクリーンショット保存済み）
    """
    max_next_clicks = 2

    for attempt in range(max_next_clicks + 1):
        # 現在表示中のタブを確認
        tabs = collect_date_tabs(page)
        for tab in tabs:
            if tab["date_str"] == target_date:
                try:
                    tab["locator"].click()
                    page.wait_for_load_state("domcontentloaded")
            ...
            for sel in (
                ...
                try:
                    ...
                    if btn.is_visible(timeout=1000):
                        btn.click()
                        page.wait_for_load_state("domcontentloaded")
                        print(f"    [INFO] >> ボタンをクリックしました（試行 {attempt + 1}）")
                        next_clicked = True
                        break
                except Exception:
                    continue
            if not next_clicked:
                print("    [INFO] >> ボタンが見つかりません。日付タブの探索を終了します。")
                break

    # 見つからなかった → スクリーンショット保存
    try:
        ss_path = f"{screenshot_dir}/date_tab_not_found_{target_date}_{int(time.time())}.png"
        page.screenshot(path=ss_path)
        visible_dates = [t["date_str"] for t in collect_date_tabs(page)]
        print(f"    [WARN] 日付タブ {target_date} が見つかりませんでした")
        print(f"           取得可能な日付: {visible_dates}")
        print(f"           SS: {ss_path}")
    except Exception:
        pass
    return False
